...
#include "heartbreak_bongo_cat_widget.h"
...
void state_widget_wpm(struct heartbreak_bongo_cat_widget *widget, int wpm) {
    LOG_DBG("anim state %d", current_anim_state);
    if (wpm < CONFIG_HEARTBREAK_BONGO_CAT_IDLE_LIMIT) {
    ...
    } else if (wpm < CONFIG_HEARTBREAK_BONGO_CAT_SLOW_LIMIT) {
...
int heartbreak_bongo_cat_widget_init(struct hearbreak_bongo_cat_widget *widget, lv_obj_t *parent) {
...
int wpm_status_listener(const zmk_event_t *eh) {
    struct heartbreak_bongo_cat_widget *widget;
...
ZMK_LISTENER(heartbreak_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(heartbreak_bongo_cat_widget, zmk_wpm_state_changed);
